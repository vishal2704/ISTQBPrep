...
const EXAM_TOUR_STEPS = [
  { selector: "[data-tour=exam-header]", title: "Keep track as you go", text: "Timer (on timed exams) and help live here — your progress bar is just below." },
  { selector: "[data-tour=exam-badges]", title: "Question info & bookmarking", text: "Chapter, difficulty and question type show here — tap the star to bookmark this question for later." },
  ...
  { selector: "[data-tour=exam-dots]", title: "Jump to any question", text: "Tap to expand the full list. Green means answered, and a star marks anything you've bookmarked." },
];
...
export default function Exam() {
  ...
  }, [examData, q, type, answered, currentIndex, tourActive, setAnswer, setCurrentIndex, setLocked, toggleBookmark]);
  ...
  return (
    <PageShell bg="exam" homePath={examData.chapter?.startsWith("tm") ? "/test-manager" : "/foundation"}>
      ...
      <Card className="mt-5" {...swipeHandlers}>
        <div data-tour="exam-badges" className="flex items-center justify-between gap-2 mb-3">
          <div className="flex items-center gap-2 flex-wrap">
            <Badge color="blue">{getChapterHeaderLabel(q.chapter)}</Badge>
          ...
          <button
            ...
            className={`text-2xl shrink-0 transition-transform hover:scale-110 ${bookmarked ? "" : "opacity-40"}`}
      ...
      <div
        className="fixed sm:static bottom-0 left-0 right-0 sm:left-auto sm:right-auto bg-surface/95 sm:bg-transparent backdrop-blur sm:backdrop-blur-none border-t sm:border-t-0 border-border px-4 sm:px-0 py-3 sm:py-0 sm:mt-5 flex items-center justify-between z-20"
      ...
      >
        ...
        <Button variant={examData.timed || answered || isLast ? "primary" : "outline"} onClick={goNext}>
          {isLast ? (examData.timed ? "Finish Exam 🏁" : "Review Answers 📝") : "Next Question →"}
        </Button>
      </div>
      {/* Reserves space so the fixed mobile bottom bar doesn't cover the
          navigator/hints that follow — desktop is unaffected (bar is static there). */}
      ...
      <p className="text-center text-inkSoft text-xs mt-3 hidden sm:block">
        <kbd className="px-1.5 py-0.5 rounded bg-black/10 font-bold">1</kbd>–
        <kbd className="px-1.5 py-0.5 rounded bg-black/10 font-bold">4</kbd> Answer ·{" "}
        <kbd className="px-1.5 py-0.5 rounded bg-black/10 font-bold">←</kbd>
        <kbd className="px-1.5 py-0.5 rounded bg-black/10 font-bold">→</kbd> Navigate ·{" "}
        <kbd className="px-1.5 py-0.5 rounded bg-black/10 font-bold">B</kbd> Bookmark
      </p>
